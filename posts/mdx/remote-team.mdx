import  {BlogImage, ImageReference, ProTip, KeyLearnings} from '../../components/blog';
...
<ProTip title={`Write, Record, Share`}>
  Make it a habit to summarize, write down and share key points from meetings via a common channel (Slack, Google Docs, etc.). If possible, record the meetings so that 
  those who cannot attend can still review them.
</ProTip>
